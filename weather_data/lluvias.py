...
path_sitka = Path('sitka_weather_2021_full.csv')
...
for row in reader:
    try:
        ...
        sitka = int(row[4])  # Lee la temperatura máxima
        if sitka:  # Verifica si el valor no está vacío
            sitka_rain.append(int(sitka))  # Convierte a entero y lo agrega a la lista
...
for rows in reader_valley:
    try:
        current_date = datetime.strptime(rows[2], '%Y-%m-%d')  # Convierte la fecha
        death_valley = float(rows[3])  # Lee la temperatura máxima
        if death_valley:  # Verifica si el valor no está vacío
            valley.append(float(death_valley))  # Convierte a entero y lo agrega a la lista
            dates.append(current_date)
        else:
            print(f"Temperatura faltante en la fecha: {current_date}")
    except ValueError:
        print(f'Missing data for death valley in row {rows} : {current_date}')
