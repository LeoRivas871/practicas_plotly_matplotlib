...
for row in reader:
    try:
        ...
        sitka = float(row[5])  # Lee la temperatura máxima
        if sitka:  # Verifica si el valor no está vacío
            sitka_rain.append(float(sitka))  # Convierte a entero y lo agrega a la lista
...
ax.set_title('Sitka Rains', fontsize=24)
